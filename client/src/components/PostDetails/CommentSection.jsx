        ...
        <div className="space-y-2">
          {comments?.map((c, i) => (
            <div className="flex items-start gap-2.5" key={i}>
              {c.user?.imageUrl ? (
                <img src={c.user?.imageUrl} className="size-10 rounded-full mr-2" alt="" />
              ) : (
                <div className="size-10 rounded-full bg-green-600">
                  <h1 className="text-center mt-2">{c.user?.name && c.user.name.charAt(0).toUpperCase()}</h1>
                </div>
              )}
              <div className="flex flex-col gap-1">
                <div className="flex flex-col w-full max-w-[386px] leading-1.5 p-4 border-gray-200 bg-gray-100 rounded-e-xl rounded-es-xl dark:bg-gray-700">
                  <div className="flex items-center space-x-2 rtl:space-x-reverse mb-2">
                    <Link to={`/creators/${c.user?.name}`}>
                      <span className="text-sm font-semibold text-gray-900 dark:text-white">{c.user?.name}</span>
                    </Link>
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">{moment(c.createdAt).fromNow()}</span>
                  </div>
                  <p className="text-sm font-normal text-gray-900 dark:text-white break-words max-w-md">{c.message}</p>
                  {c.selectedFile && (
                    <div className="group relative my-2.5">
                      ...
                      <img src={c.selectedFile} alt="comment" className="rounded-lg" />
                    </div>
                  )}
                </div>
              </div>
